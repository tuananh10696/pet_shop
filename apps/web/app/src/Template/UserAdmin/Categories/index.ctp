...
<div class="table_list_area">
	<?php if ($page_config->is_category_multilevel == 1) : ?>
		<nav aria-label="breadcrumb">
			<ul class="breadcrumb">
				<li class="breadcrumb-item"><a href="<?= $this->Url->build(['action' => 'index', '?' => ['sch_page_id' => $query['sch_page_id']]]); ?>">ルート</a>
				</li>
				...
				<?php while ($pcat = array_pop($parent_categories)) : ?>
					...
					<?php else : ?>
						<li class="breadcrumb-item"><a href="<?= $this->Url->build(['action' => 'index', '?' => ['sch_page_id' => $query['sch_page_id'], 'parent_id' => $pcat->id]]); ?>"><?= $pcat->name; ?></a>
						</li>
					<?php endif; ?>
				<?php endwhile; ?>
			</ul>
