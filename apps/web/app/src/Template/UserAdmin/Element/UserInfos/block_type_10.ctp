      ...
      <div class="editor__table" data-section-no="<?= h($content['section_sequence_id']); ?>" data-block-type="<?= h($content['block_type']); ?>">
          ...
          <?php if (false) : ?>
            <div class="table__row">
              <div style="text-align: right;float: right;">
                <span>
                  枠スタイル：<?= $this->Form->input(
                          "info_contents.{$rownum}.option_value",
                          [
                            'type' => 'select',
                ...
                <span id="idWakuColorCol_<?= h($rownum); ?>" style="<?= ($content['option_value'] == 'waku_style_6' ? 'display: none;' : ''); ?>">
                  色：<?= $this->Form->input(
                      "info_contents.{$rownum}.option_value2",
                      [
                        'type' => 'select',
        ...
        <div class="table__body " data-waku-block-type="<?= $content['block_type']; ?>">
          ...
          <div class="table__row">
            ...
            <div class="table__column list_table_sub" id="wakuId_<?= h($content['section_sequence_id']); ?>" data-section-no="<?= h($content['section_sequence_id']); ?>" data-block-type="<?= h($content['block_type']); ?>">
